...
from datautil.seriesdata.seriesdataload_simple import SeriesDatasetFile
...
def get_train_dataloader_simple(args, norm_dict):
    trdatalist = []
    names = args.img_dataset[args.dataset]
    args.domain_num = len(names)
    for i in range(len(names)):
        tmpdatay = SeriesDatasetFile(args.dataset, args.task, args.data_dir,
                                     names[i], i, test_envs=args.test_envs, mode="train", norm_dict=norm_dict).labels
        l = len(tmpdatay)
        indexall = np.arange(l)
        np.random.seed(args.seed)
        np.random.shuffle(indexall)
        ...
        trdatalist.append(SeriesDatasetFile(args.dataset, args.task, args.data_dir,
                                            ...
                                            mode="train", norm_dict=norm_dict))
    train_loaders = [InfiniteDataLoader(
        dataset=env,
        weights=None,
        batch_size=args.batch_size,
...
def get_eval_dataloader_simple(args, norm_dict):
    tedatalist = []
    names = args.img_dataset[args.dataset]
    args.domain_num = len(names)
    for i in range(len(names)):

...
def load_dataset(dataset_root="MagNet 2023 Database", material=None, mode=None):
    ...
    if mode == "train":
        ...
        h_file = os.path.join(dataset_root, material, "H_Waveform.csv")
...
class SeriesDatasetSimple(Dataset):
    ...
    def __getitem__(self, index):
        ...
        b_ = img_[2:1026][:, np.newaxis]
        h_ = img_[1026:2050][:, np.newaxis]
...
class SeriesDatasetFile(Dataset):
    ...
    def __getitem__(self, index):
        ...
        head = 0.1 * torch.ones(8, norm_h.size()[1])
...
if __name__ == '__main__':
    norm = init_norm_dict("D:\\PycharmProjects\\dg-magnet\\variables_3C90.npz")
    ...
    testData = SeriesDatasetFile(dataset='test_dataset', task='test_task',
                                 root_dir='D:\\PycharmProjects\\dg-magnet\\valid_data\\', domain_name='3C90',
                                 norm_dict=norm)
